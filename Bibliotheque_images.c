...
int pgm_extraire(int matrice[MAX_HAUTEUR][MAX_LARGEUR], int lignes1, int colonnes1, int lignes2, int colonnes2, int *p_lignes, int *p_colonnes){
	int hauteur_init = *p_lignes;
	int largeur_init = *p_colonnes;
	
	if (lignes1 > *p_lignes || lignes2 > *p_lignes || *p_lignes > MAX_HAUTEUR || colonnes1 > *p_colonnes || colonnes2 > *p_colonnes || *p_colonnes > MAX_LARGEUR){
		//printf("erreur1");
		return -1;
	}
	if (lignes1 > lignes2 || colonnes1 > colonnes2 || (lignes2 - lignes1-1) > *p_lignes || (colonnes2 - colonnes1-1) > *p_colonnes){
		//printf("erreur2");
		return -1;
	}
	
	if (lignes1 < 0 || colonnes1 < 0 || lignes2 < 0 || colonnes2 < 0 ){
		//printf("erreur2");
		return -1;
	}
	...
	int matrice_temp[hauteur+1][largeur+1];
	
	for (int i=lignes1; i<lignes2; i++){
		for (int j=colonnes1; j<colonnes2; j++){
			matrice_temp[i-lignes1][j-colonnes1] = matrice[i][j];
		}
	}
